@extends ('Admin.master') @section('TableBrand')
    <div class="row">
        ...
        <div class="col-xl-10 offset-xl-1" style="margin-top: 20px">
                    ...
                    @foreach ($Brand as $value)
                        <tr>
                            ...
                            <td scope="row" style="text-align: right" class="col-3 col-sm-3">
                                <a href="{{route('editBrand',$value->id)}}" class="btn btn-success">Sửa</a>
                                <a href="{{route('deleteBrand',$value->id)}}" onclick="return confirm('Liên Quan Khóa Ngoại. Bạn Có Chắc Muốn Xóa Không?')" id="delete" class="btn btn-danger">Delete</a> 
                            </td>
                        </tr>
                    @endforeach
                </tbody>
            ...
            <div class="col-xl-6 offset-xl-3">
                <div class="pagination" style="margin-left: 80px;">
                    @if (ceil($Brand->total()/8)>1)
                        <a href="#">«</a>
                        @for($i=1;$i<=ceil($Brand->total()/8);$i++)
                            <a class="page-item {{($i == $Brand->currentPage())?'active':''}}" href="?page={{$i}}">{{$i}}</a>
                        @endfor
                        <a href="#">»</a>
                    @else
