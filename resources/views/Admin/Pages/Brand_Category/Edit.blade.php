                ...
                <div class="form-group mr-20">
                    ...
                    <select name="brand_id" class="form-control" id="input_cate" required="required">
                        @foreach ($Brand as $value)
                            <option value="{{$value->id}}" {{$value->id == $BrandCategory->brand_id ? 'selected' : ''}}>{{$value->name}}</option>
                        @endforeach
                    </select>
                    @error('brand_id')
                ...
                <div class="form-group mr-20">
                    ...
                    <select name="category_id" class="form-control" id="input_cate" required="required">
                        @foreach ($Category as $value)
                            <option value="{{$value->id}}" {{$value->id == $BrandCategory->category_id ? 'selected' : ''}}>{{$value->name}}</option>
                        @endforeach 
                    </select>
                    @error('category_id')
                        <small class="form-text" style="color: red">{{$message}}</small>
